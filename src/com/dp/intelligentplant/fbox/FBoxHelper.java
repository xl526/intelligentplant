...
public class FBoxHelper {
	 ...
	 * @param accessToken
	 * @param boxuid
	 * @param ids  盒子下监控点uid数组
	 * @return
	 */
	public static String getBoxDmonValues(String accessToken, String boxNo, String[] ids) {
		
		String result = null;
		String url = FBoxConfig.API_BASE_URL + "v2/dmon/value/get?boxNo=" + boxNo;
		String authorization = "Bearer" + " " + accessToken;
		...
		try {
			formEntity = new StringEntity(jsonObject.toJSONString(), "UTF-8");
	...
	public static String boxGrouped(String accessToken) {
		String result = null;
		String url = "http://fbox360.com/api/client/box/grouped";
		String authorization = "Bearer" + " " + accessToken;
	...
	public static List<String> getOnlineBox(String accessToken) {
		List<String> list = new ArrayList<String>();
		String result = boxGrouped(accessToken);
		if (result.contains("Authorization has been denied for this request")) {
			accessToken = login();
		}
		result = boxGrouped(accessToken);
		JSONArray arr = JSONArray.parseArray(result);
		JSONObject json = arr.getJSONObject(1);
		JSONArray arr1 = json.getJSONArray("boxRegs");
		for (Iterator<Object> iterator = arr1.iterator(); iterator.hasNext();) {
			JSONObject json1 = (JSONObject) iterator.next();
			JSONObject json2 = json1.getJSONObject("box");
			if (json2.getIntValue("connState") == 1) {
				list.add(json2.getString("uid"));
			}
		}
		return list;
	}
